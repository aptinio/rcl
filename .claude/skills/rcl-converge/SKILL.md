...
git rev-parse --verify "$DEFAULT_BRANCH" >/dev/null || { echo "no default branch: $DEFAULT_BRANCH"; exit 1; }
   ...
   ```
2. **Claim in the foreground, then run the review detached.** Run the first block below exactly once as a foreground Bash call. Its quoted `<TARGET>` is the exact resolved accounting key (the quotes are shell syntax, not part of the value). Never set `run_in_background` on this claim and never call it separately again. Any non-zero exit is an accounting/infrastructure failure: release the target lock, report it, and stop. Exit 3 is an accounting/infrastructure failure. A successful claim is spent even if the later launch fails, is killed, produces no report, or is inconclusive.
